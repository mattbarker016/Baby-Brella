...
public class APIClient : NSObject {
    ...
    */
    public func getForecast(latitude lat: Double, longitude lon: Double, completion: (forecast: Forecast?, error: NSError?) -> Void) {
        let url = URL(string: APIClient.forecastIOURL + apiKey + "/\(lat),\(lon)?units=\(units)")!
    ...
    /**
        ...
        - parameter completion: A block that returns the `Forecast` at the latitude and longitude you specified or an error.
    */
    public func getForecast(latitude lat: Double, longitude lon: Double, time: Date, completion: (forecast: Forecast?, error: NSError?) -> Void) {
        let timeString = String(format: "%.0f", time.timeIntervalSince1970)
        let url = URL(string: APIClient.forecastIOURL + apiKey + "/\(lat),\(lon),\(timeString)?units=\(units)")!
        getForecast(url, completion: completion)
    }
    
    private func getForecast(_ url: URL, completion: (forecast: Forecast?, error: NSError?) -> Void) {
